...
export default class SquadServer extends EventEmitter {
  ...
  setupLogParser() {
    ...
    this.logParser.on('PACKET_ACKNOWLEDGMENT_WARNING', async (data) => {
		// Log information about the event
		Logger.verbose(
			'SquadServer',
			1,
			`Packet acknowledgment warning received from ${data.playerName} (EOS: ${data.eosID}, SteamID: ${data.steamID}), IP Address: ${data.ipAddress}`
		);

		// Do additional processing if needed

		// Emit an event if needed
		this.emit('PACKET_ACKNOWLEDGMENT_WARNING', data);
	});

    this.logParser.on('ITEM_SPAWN_ATTEMPT', async (data) => {
      // Log information about the event
      Logger.verbose(
        'SquadServer',
        1,
        `Item spawn attempt detected: Player ${data.playerName} - EOSID: ${data.playerEOSID} - SteamID: ${data.playerSteamID} - Time: ${data.time} - Attempted Item: ${data.item}`
      );

      // Do additional processing if needed
      // For example, you can fetch additional player information or emit an event
      ...
      // Emit an event if needed
      this.emit('ITEM_SPAWN_ATTEMPT', data);
    });
